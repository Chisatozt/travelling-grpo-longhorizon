...
The existing corpus is ``sft/travel_sft_public.json`` (currently 244
...
except ImportError:  # direct module loading or ``python sft/merge_travel_sft.py``
    try:
...
try:  # ``python sft/merge_travel_sft.py``
...
except ModuleNotFoundError:  # importing as ``sft.merge_travel_sft`` in tests
    from sft.prepare_travel_sft import (
        _is_travel_record,
        _iter_source_records,
        _record_has_think,
        _report_is_trainable,
...
def _token_lengths(
    ...
    if tokenizer_name:
        ...
        from sft.qwen35_mask import exact_assistant_token_mask, native_template_ids
...
def main() -> None:
    ...
    parser.add_argument("--base", default="sft/travel_sft_public.json", help="Original 244-record ShareGPT corpus (never overwritten).")
    parser.add_argument("--input", nargs="+", required=True, help="DeepSeek Teacher JSON/JSONL/cache files.")
    parser.add_argument("--output", default="sft/travel_sft_canonical.jsonl", help="Merged canonical JSONL output.")
    ...
    protected_base = (Path(__file__).resolve().parent / "travel_sft_public.json").resolve()
    ...
    resolver = TravelTaskResolver(
        project_root=Path(__file__).resolve().parents[1], task_paths=task_paths, explicit_map=alignment_map
    )
    ...
    if args.task_pool_manifest:
        ...
        for input_path in (Path(value) for value in args.input):
            ...
            if isinstance(payload, dict) and payload.get("schema_version") == "travelgym-teacher-cache-v1":
                ...
                try:
                    from eval.teacher_collection import teacher_pass_key
    ...
    if args.split_output_dir:
        ...
        try:
            from .sft_split import build_sft_split
        except ImportError:
            from sft_split import build_sft_split
        build_sft_split(
            merged,
