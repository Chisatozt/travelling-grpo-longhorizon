...
# Match ``eval/build_test_manifests.py`` so the validation smoke view is the
...
def is_permanently_quarantined_opaque(
    ...
    *,
    ...
    return normalized.endswith("/sft/travel_sft_public.json") or normalized == "sft/travel_sft_public.json"
...
def build_task_pool_manifest(
    *,
    project_root: str | Path,
    sft_source: str | Path = "sft/travel_sft_public.json",
    task_map: Mapping[str, str] | None = None,
...
def main() -> None:
    ...
    parser.add_argument("--sft-source", type=Path, default=Path("sft/travel_sft_public.json"))
