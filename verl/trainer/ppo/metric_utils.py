...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    # Interaction rollouts carry a private terminal scalar in non-tensor
    # metadata.  Log it directly instead of deriving misleading per-turn
    # sums/maxima from conversation histories.
    raw_terminal = batch.non_tensor_batch.get("reward_scores") if hasattr(batch, "non_tensor_batch") else None
    terminal_values = []
    if raw_terminal is not None:
        if hasattr(raw_terminal, "tolist"):
            raw_terminal = raw_terminal.tolist()
        if not isinstance(raw_terminal, (list, tuple)):
            raw_terminal = [raw_terminal]
        for item in raw_terminal:
            while isinstance(item, (list, tuple)) and len(item) == 1:
                item = item[0]
            terminal_only = False
            if isinstance(item, dict):
                try:
                    terminal_only = bool(float(item.get("interact_with_env_terminal_only", 0.0)))
                except (TypeError, ValueError):
                    terminal_only = False
            if isinstance(item, dict) and "interact_with_env" in item and terminal_only:
                try:
                    terminal_values.append(float(item["interact_with_env"]))
                except (TypeError, ValueError):
                    pass
    if terminal_values:
