...
class OpenAIFunctionParsedSchema(BaseModel):
    ...
    name: str
    # SGLang releases have returned both the wire JSON string and an already
    # decoded mapping.  Keep the boundary permissive; canonical TravelGym
    # records still store a dict and OpenAI requests still serialize it to a
    # JSON string in the adapter.
...
class OpenAIFunctionCallSchema(BaseModel):
    ...
    def from_openai_function_parsed_schema(parsed_schema: OpenAIFunctionParsedSchema) -> tuple["OpenAIFunctionCallSchema", bool]:
        ...
        if isinstance(parsed_schema.arguments, dict):
            arguments = dict(parsed_schema.arguments)
        else:
            try:
