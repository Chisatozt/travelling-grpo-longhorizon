...
class CompletionCallback(ABC):
    ...
    def _apply_chat_template(self, messages, **kwargs):
        """Render with the native Qwen thinking policy when supported.

        The scheduler is also used with older Transformers tokenizers that do
        not expose ``enable_thinking``.  Keep the compatibility fallback, but
        make the Qwen3.5 path explicit instead of silently using a different
        rendering policy.
        """
    ...
    def extra_body(self) -> Dict[str, Any]:
        """Extra body passed to the OpenAI-compatible API.

        The request carries the already canonical message/tool schema, but
        Qwen3.5 still needs its request-scoped native thinking flag.  Keeping
        this here makes the alternate HTTP chat scheduler use the same
        ``enable_thinking=True`` policy as the SGLang token-ID path.  Other
        models receive no provider-specific fields.
        """
        try:
            model_path = str(self.config.actor_rollout_ref.model.path)
        except AttributeError:
            model_path = ""
        if "qwen3" in model_path.casefold():
...
class ToolCompletionCallback(CompletionCallback):
    ...
    async def _call_tool(self, tool_call) -> Dict[str, str]:
        ...
        try:
            # TravelGym's InteractTool returns a six-field compatibility tuple
            # (feedback, step_reward, done, choice, content, metrics), while
            # ordinary VERL tools return the historical three fields.  The
            # scheduler only needs public feedback here; reward/diagnostics
            # remain trainer-private and are deliberately not interpolated
            # into the tool message.
            result = await tool.execute(instance_id, tool_args)
            if isinstance(result, (tuple, list)):
                if not result:
                    raise ValueError("tool returned an empty result")
                tool_response = result[0]
                tool_metrics = result[-1] if len(result) >= 3 else {}
            else:
                tool_response, tool_metrics = result, {}
    ...
    def postprocess(self, batch: DataProto, batch_conversations: List[List[Dict[str, str]]], n: int) -> DataProto:
        ...
        prompts = [self._apply_chat_template(prompt, tools=self.tool_schemas, add_generation_prompt=True, tokenize=False) for prompt in batch.non_tensor_batch["raw_prompt"]]
        assert len(batch_conversations) == len(prompts) * n

        # sequences: [prompt + response]
        sequences = [self._apply_chat_template(conversation, tools=self.tool_schemas, add_generation_prompt=False, tokenize=False) for conversation in batch_conversations]
