...
def get_tool_call_parser_type(tokenizer: PreTrainedTokenizer, config=None) -> str:
    ...
    if requested:
        ...
        if parser.bot_token not in vocab or (parser.eot_token and parser.eot_token not in vocab):
...
class AsyncSGLangRollout(BaseRollout):
    ...
    async def _async_rollout_a_request(self, req: AsyncRolloutRequest, do_sample: bool = True, is_validate: bool = False, **kwargs) -> AsyncRolloutRequest:
            ...
            if _req.state == AsyncRolloutRequestStateEnum.PENDING:
                ...
                if _req.messages[-1].tool_calls is not None:
                    ...
                    for i, (tool_call, result) in enumerate(zip(parsed_tool_calls, tool_call_results)):
                        ...
                        # metadata and a stop flag.  Only response text enters
                        # the model transcript.
                        resp, reward, metrics = result[0], result[1], result[-1]
                        stop = bool(result[2]) if len(result) >= 6 else False
        ...
        async def calc_reward_and_release_fn(name: str, tool: BaseTool):
            ...
            # Keep the scalar under the tool name for backwards compatibility;
            # validity/version are trainer-only side metadata.
            ...
            for rollout_offset in range(n):
                if self._tool_schemas:
                    ...
                    if len(_input_ids) > self.config.prompt_length:
                        # Keep Qwen3.5 native spans intact.  Clipping here can
                        # cut a tool schema/reasoning marker and silently
                        # desynchronise the Actor transcript from the
