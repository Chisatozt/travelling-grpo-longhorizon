...
def get_tool_call_parser_type(tokenizer: PreTrainedTokenizer, config=None) -> str:
    ...
    if requested:
        ...
        if parser_cls is None:
            ...
            raise ValueError(f"Configured SGLang tool_call_parser={requested!r} is unavailable (available: {available})")
        parser = parser_cls()
            ...
            if _req.state == AsyncRolloutRequestStateEnum.PENDING:
                ...
                if _req.messages[-1].tool_calls is not None:
                    ...
                    for i, (tool_call, result) in enumerate(zip(parsed_tool_calls, tool_call_results)):
                        # Support both the legacy three-field tool contract and
                        # InteractTool's six-field contract.
                        resp = result[0]
                        reward = result[1]
                        metrics = result[-1]
                        stop = result[2] if len(result) >= 6 else False
                        ...
                        if len(_req.input_ids) >= self.config.max_model_len:
                            break
                    if finish_reason_type == FinishReasonTypeEnum.STOP or len(_req.input_ids) >= self.config.max_model_len:
                        finish_reason_type = FinishReasonTypeEnum.STOP
                        break
                    _req.state = AsyncRolloutRequestStateEnum.RUNNING
                else:
            ...
            for rollout_offset in range(n):
                if self._tool_schemas:
                    ...
                    if len(_input_ids) > self.config.prompt_length:
                        # A native-template prefix cannot be safely clipped:
                        # it may cut a tool schema or a reasoning marker and
                        # would make the Actor/environment transcript differ.
                        # Surface the overlength row instead of silently
                        # truncating it.
                        raise ValueError(
                            f"Prompt {data_idx} length {len(_input_ids)} exceeds "
                            f"max_prompt_len {self.config.prompt_length}; truncation=error"
                        )
                else:
