...
try:
    from sft.task_pools import load_pool_manifest, pool_task_keys, TaskPoolError
except ImportError:  # direct execution from examples/data_preprocess
    import sys
...
TOOL_CONFIG_PATH = PROJECT_ROOT / "examples" / "sglang_multiturn" / "config" / "tool_config" / "interact_tool_config.yaml"
...
def load_dataset_split(
    ...
    *,
    ...
    parquet_path = PROJECT_ROOT / "data" / dataset_name / f"{split}.parquet"
...
def main() -> None:
    ...
    parser.add_argument("--train_output", type=Path, default=PROJECT_ROOT / "data" / "alltrain_multiturn")
    parser.add_argument("--test_output", type=Path, default=PROJECT_ROOT / "data" / "alltest_multiturn")
    parser.add_argument(
        "--task-pool-manifest",
        type=Path,
