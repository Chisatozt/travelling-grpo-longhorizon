...
# TravelGym can be used directly (without going through eval/eval.py), so it
...
class TravelGymConfig:
    ...
    model_name: str = field(
        default_factory=lambda: (
            os.getenv("USER_MODEL_NAME")
            or os.getenv("ACTOR_MODEL_NAME")
            or "gpt-4o"
        )
    )
    ...
    preference_correct_reward: float = 0.0
    choice_best_reward: float = 0.0
    choice_correct_reward: float = 0.0
    ...
    # Soft protocol default: the public reducer permits an answer after search
    ...
    def __post_init__(self):
        ...
        if self.data_path is None:
            ...
            self.data_path = os.path.join(current_dir, "data", "travelgym_data_22.json")
    ...
    def validate(self):
        ...
        if self.search_failure_interval <= 0:
            raise ValueError("search_failure_interval must be positive")

        if self.elicitation_interval <= 0:
            raise ValueError("elicitation_interval must be positive")
