...
if str(REPOSITORY_ROOT) not in sys.path:
    # Add the repository root so the local veRL package resolves when the
    # source-layout package is invoked from an arbitrary working directory.
...
from ..collection.task_pools import (
    TaskPoolError,
    assert_task_pools_disjoint,
    load_pool_manifest,
)
from ..collection.teacher_collection import (
    TeacherCache,
    TeacherCacheError,
    code_revision,
    make_provenance,
...
PROJECT_ROOT = os.getenv("PROJECT_ROOT", str(REPOSITORY_ROOT / "outputs" / "evaluation"))
DEFAULT_MANIFEST_PATH = REPOSITORY_ROOT / "data" / "evaluation" / "travel_manifest.json"
DEFAULT_TEST_SET_MANIFEST_PATH = REPOSITORY_ROOT / "data" / "evaluation" / "test_manifests" / "final200.json"
TOOL_SCHEMA_PATH = REPOSITORY_ROOT / "configs" / "tools" / "interact_tool_schema.yaml"
SUPPORTED_ENVS = (
    "travel22",
    "travel33",
    "travel44",
...
async def load_data(env_name, one_choice=True, split="test", task_ids=None):
    ...
    path = REPOSITORY_ROOT / "data" / "grpo" / f"{env_name}_multiturn_onechoice" / f"{split}.parquet"
...
async def main():
    ...
    function = yaml.safe_load(TOOL_SCHEMA_PATH.read_text(encoding="utf-8"))["tool_schema"]
    ...
    if args.task_pool_manifest:
        ...
        if args.sft_collection and not strict_identity:
            raise ValueError(
                ...
                "regenerate it with travel_grpo.collection.task_pools if an old preview manifest was supplied"
            )
        if args.sft_collection:
